...
export class ShareModule {
  diqu =  // 地址数据
  {
    ...
    '0,1': { 2: '北京市' },
    '0,1,2': { 3: '东城区', 4: '西城区', 5: '朝阳区', 6: '丰台区', 7: '石景山区', 8: '海淀区', 9: '门头沟区', 12: '房山区', 13: '通州区', 14: '顺义区', 15: '昌平区', 16: '大兴区', 17: '怀柔区', 18: '平谷区', 19: '密云区', 20: '延庆区' },
    '0,22': { 23: '天津市' },
  ...
  //手机验证
  mobleValidator(c:FormControl){
    let MOBILE_REGEXP = /^1(3|4|5|7|8)\d{9}$/;
    return MOBILE_REGEXP.test(c.value) ? null : {mobile: true}
  }
  //交易数额验证 100的整数倍
  sumValidator(c:FormControl){
    let SUM_REGEXP = /^\+?[1-9][0-9]*0{2}$/;
    return SUM_REGEXP.test(c.value) ? null : {sum: true}
  }
  //两位小数验证
  flValidator(c:FormControl) {
    if(c.value) {
      var FL_REGEXP = /(^[+]?[1-9]\d*(\.\d{1,2})?$)|(^[+]?[0]{1}(\.\d{1,2})?$)/;
      return FL_REGEXP.test(c.value) ? null : {fl: true}
    }
  }
  //正整数验证
  IntValidator(c:FormControl) {
    if(c.value) {
      var FL_REGEXP = /^\+?[1-9][0-9]*$/;
      return FL_REGEXP.test(c.value) ? null : {intNum: true}
    }
  }
  //身份证验证
  idValidator(c:FormControl){
    if(c.value){
      ...
      return reg.test(c.value) ? null : {idlength: true}
    }
  }
  //一级密码长度6-16位
  passFValidator(c:FormControl){
    let fpass=/^[a-zA-z\d]{6,16}$/;
    return fpass.test(c.value) ? null : {pass: true}
  }
  passSValidator(c:FormControl){
    let spass = /^\d{6}$/;
    return spass.test(c.value) ? null : {pass: true}
  }

  mask(str, start, length, mask_char) {
  ...
  // //用户名
  // userName(c:FormControl){
  //     let UName = /^[A-z][0-9A-z]{3,9}$/;
  //     return UName.test(c.value) ? null : {UserName: true}
  // }
