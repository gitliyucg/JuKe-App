...
/**
 * Generated class for the StockdistlistPage page.
 *
 * See http://ionicframework.com/docs/components/#navigation for more info
 * on Ionic pages and navigation.
 */
@Component({
  selector: 'page-stockdistlist',
  templateUrl: 'stockdistlist.html',
})
export class StockdistlistPage {
  ...
  ionViewDidLoad() {
    ...
    this.storage.get('userStorage').then(value => {
      ...
      this.file.loading.present().then(() => {
        this.http.get(this.file.localUrl + '/action/GuQuan/GetGuQuanList/' + this.UID + '?num=' + this.Pagenum).toPromise().then(response => {
  ...
  doInfinite(infiniteScroll, UID){
    if (this.key) {
      setTimeout(() => {
        this.Pagenum++;
        this.http.get(this.file.localUrl + '/action/GuQuan/GetGuQuanList/' + UID + '?num=' + this.Pagenum).toPromise().then(response => {
